...
fetch_config_command = "#{node['cfncluster']['cookbook_virtualenv_path']}/bin/aws s3api get-object --bucket #{node['cfncluster']['cluster_s3_bucket']} --key #{node['cfncluster']['instance_types_data_s3_key']}"\
                       " --region #{node['cfncluster']['cfn_region']} #{node['cfncluster']['instance_types_data_path']}"
execute "copy_cluster_config_from_s3" do
  command fetch_config_command
  retries 3
  retry_delay 5
...
execute "generate_pcluster_slurm_configs" do
  command "#{node['cfncluster']['cookbook_virtualenv_path']}/bin/python #{node['cfncluster']['scripts_dir']}/slurm/pcluster_slurm_config_generator.py"\
          " --output-directory /opt/slurm/etc/ --template-directory #{node['cfncluster']['scripts_dir']}/slurm/templates/ --input-file #{node['cfncluster']['cluster_config_path']}  --instance-types-data #{node['cfncluster']['instance_types_data_path']}"
end
