...
def generate_slurm_config_files(output_directory, template_directory, input_file, instance_types_data_path, dryrun):
    ...
    queues = cluster_config["Scheduling"]["Slurm"]["Queues"]
    ...
    for template_name in ["slurm_parallelcluster.conf", "slurm_parallelcluster_gres.conf"]:
        _generate_slurm_parallelcluster_configs(
            queues,
            head_node_config,
            cluster_config["Scheduling"]["Slurm"]["Settings"],
            template_name,
            env,
            output_directory,
            dryrun,
...
def _vcpus(compute_resource) -> int:
    ...
    return (
        vcpus_count
        if not disable_simultaneous_multithreading
        else (vcpus_count // threads_per_core())
    )
...
def main():
    try:
        ...
        generate_slurm_config_files(args.output_directory, args.template_directory, args.input_file, args.instance_types_data, args.dryrun)
